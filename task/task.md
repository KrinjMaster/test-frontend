...
<img src="https://github.com/Central-University-IT/playground-frontend/blob/main/task/resume.png" width="600" alt="Пример резюме">
...
### Интересы:
Поле, куда можно вписать кратко основные интересы Васи. Он отметил, что интересов у него может быть много, поэтому в форме нужно добавить кнопку для добавления дополнительных полей ввода интересов.
Если Вася не указал, чем интересуется, заголовок &laquo;Интересы&raquo; также не нужен.

### Языки:
Поле для языков. Причем сам язык и уровень владение языком он хочет вписывать в отдельные поля для ввода, и хочет такую же кнопку с добавлением новых полей для ввода языка и уровня владения.
...
Так как у Васи было несколько прошлых работ, он хотел бы, чтобы была кнопка добавления еще одного места работы, где были бы точно такие же поля для ввода.
...
Так как Вася может иметь несколько образований, он бы хотел, чтобы была кнопка добавления еще одного блока об образовании, где были бы точно такие же поля для ввода.
...
Так как Вася мог пройти несколько курсов, он хотел бы, чтобы была кнопка добавления еще курсов, где были бы точно такие же поля для ввода.
...
- Для каждого резюме должна быть предусмотрена кнопка &laquo;Действия&raquo;, которую можно нажать для отображения выпадающего меню с доступными действиями. Текст на кнопке не важен (например, это может быть “...”), но она должна иметь title = “Действия”
- В выпадающем меню должны присутствовать действия “Открыть” и &laquo;Удалить&raquo;
...
- На странице со списком резюме должна присутствовать кнопка для добавления курса. Визуально на кнопке должен быть текст “+”, также она должна иметь title = "Добавить"
- При нажатии на кнопку &laquo;Добавить&raquo; происходит переход на страницу создания резюме из Части 1
- На странице просмотра созданного резюме должна присутствовать кнопка “Сохранить” для сохранения результата и возврата в список всех резюме
...
Вася страшно не любит современные фреймворки, и поэтому вас тоже просит сделать конструктор на чистом JS, HTML и CSS. Также ваш друг критически настроен к NPM, поэтому вы не можете ставить никакие сторонние пакеты – можно пользоваться только тем, что вам предоставил Вася.
...
Для экрана резюме помимо интеграционных есть и скриншотные тесты, поэтому ваша задача – повторить предоставленный макет: сделать те же отступы, цвета, расположение блоков. Экран конструктора и список резюме проверяются только интеграционными тестами, и дизайн оцениваться не будет. Однако, чтобы автотесты проходили, очень важно расставить элементам страницы правильные `test-id`, см. следующий раздел.

При необходимости код может быть проверен экспертами вручную. Эта проверка является дополнительной и не отменяет того, что тесты должны проходить.
...
Для кнопки добавления новой работы указать атрибут `test-id=”add-course”`
...
Для каждого содержательного блока (Личные данные, Интересы, Языки, Описание ниже заголовка, Опыт работы, Обучение и квалификация, Курсы) то есть блока содержащий в себе соответствующий контент, сделать `test-id="resume-main-section"`
